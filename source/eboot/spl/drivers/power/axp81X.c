...
static axp_contrl_info axp_ctrl_tbl[] = {
	...
	{ "dcdc2", 500, 1300, 0x21, 0x7f, 10, 1200, 20,
	  BOOT_POWER81X_OUTPUT_CTL1, 1 },
	{ "dcdc5", 800, 1840, 0x24, 0x7f, 10, 1120, 20,
	  BOOT_POWER81X_OUTPUT_CTL1, 4 },
...
{
	u8 reg_val = 0;
	if (pmic_bus_read(AXP81X_RUNTIME_ADDR, BOOT_POWER81X_HOTOVER_CTL,
			  &reg_val)) {
		return -1;
	}
	...
	/* for watchdog reset */
	reg_val |= 1;
	if (pmic_bus_write(AXP81X_RUNTIME_ADDR, BOOT_POWER81X_HOTOVER_CTL,
			   reg_val)) {
		return -1;
	}
	return 0;
...
static inline void disable_dcdc_pfm_mode(void)
{
	...
	pmic_bus_read(AXP81X_RUNTIME_ADDR, 0x80, &val);
	val |= (0x01 << 3); /*dcdc4 for gpu pwm mode*/
	val |= (0x01 << 4); /*dcdc5 for dram pwm mode*/
	pmic_bus_write(AXP81X_RUNTIME_ADDR, 0x80, val);

	/* disable dcm mode for GPU stability Vdrop issue*/
	pmic_bus_write(AXP81X_RUNTIME_ADDR, 0xff, 0x0);
	pmic_bus_write(AXP81X_RUNTIME_ADDR, 0xf4, 0x6);
	pmic_bus_write(AXP81X_RUNTIME_ADDR, 0xf2, 0x4);
	pmic_bus_write(AXP81X_RUNTIME_ADDR, 0xf5, 0x4);
	pmic_bus_write(AXP81X_RUNTIME_ADDR, 0xff, 0x1);
	pmic_bus_write(AXP81X_RUNTIME_ADDR, 0x12, 0x40);
	pmic_bus_write(AXP81X_RUNTIME_ADDR, 0xff, 0x0);
}
...
int axp81X_probe_power_key(void)
{
	...
	if (pmic_bus_read(AXP81X_RUNTIME_ADDR, PMU_POWER_KEY_STATUS,
			  &reg_value)) {
		return -1;
	}
	/* POKLIRQ,POKSIRQ */
	...
	if (reg_value) {
		if (pmic_bus_write(AXP81X_RUNTIME_ADDR, PMU_POWER_KEY_STATUS,
				   reg_value)) {
			return -1;
		}
	}
