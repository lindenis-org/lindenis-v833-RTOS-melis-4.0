	...
	if (boot0_para & BOOT0_PARA_USE_INTERNAL_DEFAULT_TIMING_PARA) {
		...
		if (boot0_para & BOOT0_PARA_USE_EXTERNAL_INPUT_TIMING_PARA) {
			...
			if (dly == 0xff) {
				...
				} else if (spd_md_id == HS200_SDR104) {
					if (freq_id <= CLK_25M) {
						dly = 0;
					} else {
						mmcinfo("wrong freq %d at spd md %d\n", freq_id, spd_md_id);
...
static int mmc_trans_data_by_dma(struct mmc *mmc, struct mmc_data *data)
{
	...
	for (i = 0; i < buff_frag_num; i++, des_idx++) {
		...
		pdes[des_idx].buf_addr_ptr1 =
			((size_t)buff + i * SDXC_DES_BUFFER_MAX_LEN);
		if (i == 0)
		...
		if (i == buff_frag_num - 1) {
			...
			pdes[des_idx].buf_addr_ptr2 =
				((size_t)&pdes[des_idx + 1]);
		}
		mmcdbg("mmc %d frag %d, remain %d, des[%u](%x): "
		       "[0] = %x, [1] = %x, [2] = %x, [3] = %x\n",
	...
	writel(((size_t)pdes), &mmchost->reg->dlba);
	writel((2U << 28) | (7 << 16) | 248,
...
//	u8 ext_f_max = priv_info->boot_mmc_cfg.boot_hs_f_max;
	...
	if (!mmc_get_timing_cfg(sdc_no, 2, 2, &odly, &sdly)) {
		...
		mmc->f_max_ddr = 25000000;
	if ((priv_info->ext_para1 & (EXT_PARA1_1V8_GPIO_BIAS | BOOT0_SUP_HS))
		== (EXT_PARA1_1V8_GPIO_BIAS | BOOT0_SUP_HS)) {
		if (!mmc_get_timing_cfg(sdc_no, 3, 4, &odly, &sdly)) {
			if ((odly != 0xff) && (sdly != 0xff)) {
				mmc->host_caps |= MMC_MODE_HS200;
				mmc->f_max = 150000000;
				mmcinfo("---set f_max to 150M\n");
			} else {
				mmc->f_max = 100000000;
			}
		} else if (!mmc_get_timing_cfg(sdc_no, 3, 3, &odly, &sdly)) {
			if ((odly != 0xff) && (sdly != 0xff)) {
				mmc->host_caps |= MMC_MODE_HS200;
				mmc->f_max = 100000000;
				mmcinfo("---set f_max to 100M\n");
			} else {
				mmc->f_max = 50000000;
			}
		} else if (!mmc_get_timing_cfg(sdc_no, 3, 2, &odly, &sdly)) {
			if ((odly != 0xff) && (sdly != 0xff)) {
				mmc->host_caps |= MMC_MODE_HS200;
				mmc->f_max = 50000000;
				mmcinfo("---set f_max to 50M\n");
			}
		} else
			mmc->f_max = 50000000;

		if (!mmc_get_timing_cfg(sdc_no, 4, 3, &odly, &sdly)) {
			if ((odly != 0xff) && (sdly != 0xff)) {
				mmc->host_caps |= MMC_MODE_HS400;
				mmc->f_max_ddr = 100000000;
				mmcinfo("---set f_max_ddr to 100M\n");
			} else {
				mmc->f_max_ddr = 50000000;
			}
		} else if (!mmc_get_timing_cfg(sdc_no, 4, 2, &odly, &sdly)) {
			if ((odly != 0xff) && (sdly != 0xff)) {
				mmc->host_caps |= MMC_MODE_HS400;
				mmc->f_max_ddr = 50000000;
				mmcinfo("---set f_max_ddr to 50M\n");
			}
		} else
			mmc->f_max_ddr = 50000000;
	}
...
/*
fix me! when use dcache, card burn will fail. So I have to disable dcache.
A better way is adapt dcache to mmc. Hope someone to do that!
*/
static void mmc_dcache_disable(void)
{
