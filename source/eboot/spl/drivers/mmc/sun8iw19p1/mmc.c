...
int mmc_switch_to_ds(struct mmc *mmc)
{
	int err;
	...
	if (!(mmc->cardtype & EXT_CSD_CARD_TYPE_HS_26)) {
...
int mmc_switch_to_hs(struct mmc *mmc)
{
	int err;
	...
	if (!(mmc->host_caps & HSSDR52_SDR25)) {
		mmcinfo("host not support hs\n");
		return -1;
	}

	if (!(mmc->cardtype & (EXT_CSD_CARD_TYPE_HS|EXT_CSD_CARD_TYPE_DDR_1_8V))) {
...
int mmc_switch_to_hs200(struct mmc *mmc)
{
	int err;
...
int mmc_switch_to_hs400(struct mmc *mmc)
{
	int err;
	...
	if (!(mmc->host_caps & MMC_MODE_HS400)) {
...
int mmc_switch_speed_mode(struct mmc *mmc, int spd_mode)
{
	...
	if (spd_mode == DS26_SDR12)
		ret = mmc_switch_to_ds(mmc);
	else if (spd_mode == HSSDR52_SDR25)
		ret = mmc_switch_to_hs(mmc);
	else if (spd_mode == HS200_SDR104)
		ret = mmc_switch_to_hs200(mmc);
...
static int mmc_check_buswidth(struct mmc *mmc, u32 emmc_hs_ddr, u32 bus_width)
{
	...
	mmcdbg("%s: bus:%d, ddr:%d, spd_md: %d\n", __FUNCTION__, bus_width, emmc_hs_ddr ? 1 : 0, mmc->speed_mode);
...
int mmc_switch_bus_width(struct mmc *mmc, int spd_mode, int width)
{
	int err = 0;
	int emmc_hs_ddr = 0;
	u32 set_val = 0;
...
int mmc_switch_bus_mode(struct mmc *mmc, int spd_mode, int width)
{
	int err = 0;
	int tmp_spd_md = 0;
	...
	err = mmc_switch_speed_mode(mmc, tmp_spd_md);
	if (err) {
		mmcinfo("switch speed mode fail\n");
		return err;
	}

	err = mmc_switch_bus_width(mmc, spd_mode, width);
	if (err) {
...
int mmc_change_freq(struct mmc *mmc)
{
	...
	mmc->cardtype = ext_csd[196] & 0xff;

	/* switch to specific speed mode */
	if (mmc->host_caps & MMC_MODE_HS400) {
		/* firstly, switch to HS-DDR 8 bit */
		err = mmc_switch_bus_mode(mmc, HSDDR52_DDR50, bus_width);
		if (err) {
			mmcinfo("switch to DDR speed mode failed\n");
			goto OUT;
		}
		/* then, switch to HS400 */
		err = mmc_switch_bus_mode(mmc, HS400, bus_width);
		if (err) {
			mmcinfo("switch to HS400 speed mode failed\n");
			goto OUT;
		}

	} else if (mmc->host_caps & MMC_MODE_HS200) {
		err = mmc_switch_bus_mode(mmc, HS200_SDR104, bus_width);
		if (err) {
			mmcinfo("switch to HS200 speed mode failed\n");
			goto OUT;
		}
	} else if (mmc->host_caps & MMC_MODE_DDR_52MHz) {
		err = mmc_switch_bus_mode(mmc, HSDDR52_DDR50, bus_width);
		if (err) {
			mmcinfo("switch to SDR speed mode failed\n");
			goto OUT;
		}
	} else if (mmc->host_caps & MMC_MODE_HS_52MHz) {
		err = mmc_switch_bus_mode(mmc, HSSDR52_SDR25, bus_width);
		if (err) {
			mmcinfo("switch to SDR speed mode failed\n");
			goto OUT;
		}
...
int mmc_startup(struct mmc *mmc)
{
	...
	if (!IS_SD(mmc) && (mmc->version >= MMC_VERSION_4)) {
		...
		err = mmc_send_ext_csd(mmc, ext_csd);
		if (err) {
			mmcinfo("%s, mmc send ext csd failed!\n", __FUNCTION__);
			goto ERR;
		}
		mmc->ext_csd = ext_csd;
		if (!err) {
