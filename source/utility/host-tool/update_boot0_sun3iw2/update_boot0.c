...
int update_sdcard_info(char  *buf)
{
	...
	card_info->card_ctrl_num = 1;

        card_info->line_count[i] = 6;

	if (!script_parser_fetch("card_boot", "logical_start", value))
        {
            card_info->boot_offset = value[0];
	    printf("boot_offset = %d\n", card_info->boot_offset);
        }
...
int update_for_boot0(char *boot0_name, int storage_type)
{
    ...
    if (script_parser_sunkey_all("dram_para", &boot0_head->prvt_head.dram_para))
    {
    ...
    {
        boot0_head->prvt_head.debug_log = value[0];
    }
