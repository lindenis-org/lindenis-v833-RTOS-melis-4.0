...
int _tcstol(const char *csTmp, int n);
string Mid(string str, int nFirst);
string Mid(string str, int nFirst, int count);

char *GetBuffer(CString str, int fileLength)
{
    int len = str.length();
    //if(len < fileLength)
    len = fileLength + 1;
...
bool CMarkup::ReadTextFile(LPCTSTR szFileName, CString &strDoc, CString *pstrError, int *pnFlags)
{
    ...
    {
        char *copy_data;
        copy_data = GetBuffer(strDoc, nFileByteLen);
        fread(copy_data, nFileByteLen, 1, fp);
        string result(copy_data);
        strDoc.swap(result);
        free(copy_data);
    }
    if (pstrError)
        (*pstrError) = FormatString(pstrError, _T("%s%d bytes"), szDescBOM, nFileByteLen);
#endif
    fclose(fp);
    if (pnFlags)
        *pnFlags = nFlags;
...
bool CMarkup::WriteTextFile(LPCTSTR szFileName, CString &strDoc, CString *pstrError, int *pnFlags)
{
    ...
    if (nDocLength)
    {
        ...
        nDocLength = strDocWrite.length();
        bSuccess = (fwrite((LPCTSTR)strDocWrite.c_str(), nDocLength, 1, fp) == 1);
    }
    if (pstrError)
        (*pstrError) = FormatString(pstrError, _T("%s%d bytes"), szDescBOM, nDocLength);
#endif
...
bool CMarkup::FindElem()
{
    ...
    if (m_aPos.GetSize())
    {
        int iPos = 0;
        if (m_iPos)
            iPos = m_aPos[m_iPos].iElemNext;
        else
            iPos = m_aPos[ m_iPosParent].iElemChild;
        if (iPos)
        {
            // Assign new position
            x_SetPos(m_aPos[iPos].iElemParent, iPos, 0);
...
CString CMarkup::EscapeText(LPCTSTR szText, int nFlags)
{
    ...
    _TCHAR *pDest = GetBuffer(strText, nDestSize);
    int nLen = 0;
    ...
    while (cSource)
    {
        if (nLen > nDestSize - 6)
        {
            ...
            pDest = GetBuffer(strText, nDestSize);
        }
...
CString CMarkup::UnescapeText(CString str, int nTextLength /*=-1*/)
{
    ...
    //_TCHAR* pDest = GetBuffer( strText,nTextLength );
    _TCHAR *pDest;
    _TCHAR pDest_data[nTextLength + 1];
    pDest = pDest_data;
    int nLen = 0;
    int nCharLen;
    int nChar = 0;
    ...
    memset(pDest + nTextLength, 0, 1);

    CString result(pDest);
    strText.swap(result);
    //free(pDest);
    return strText;
}
...
#endif
    for (int nChar = 0; nChar < strError.length(); ++nChar)
        if (strError[nChar] == '\r' || strError[nChar] == '\n')
        {
            strError = strError.substr(0, nChar); // no trailing newline
            break;
        }
    return strError;
}
...
int CMarkup::x_ParseElem(int iPosParent, TokenPos &token)
{
    ...
    token.nNext = 0;
    //m_strError.Empty();
    m_strError.erase(0, m_strError.length());
    ...
    while (1)
    {
        ...
        else if (nTypeFound == 0)   // end tag
        {
            ...
            if (nMatchDepth == 0)
            {
                ...
                if (m_strError.empty())
                {
                    m_strError = FormatString(&m_strError, _T("No start tag for end tag '%s' at offset %d"),
        ...
        if (nMatchDepth || nTypeFound == -2)
        {
            ...
            while (nDepth > nMatchDepth)
            {
                ...
                {
                    if (nTypeFound == 0)
                        m_strError = FormatString(&m_strError, _T("End tag '%s' at offset %d does not match start tag '%s' at offset %d"),
                                                  x_GetToken(token).c_str(), token.nL - 1, aNodes.Top().strMeta.c_str(), pElem->nStart);
                    else
                        m_strError = FormatString(&m_strError, _T("Element '%s' at offset %d not ended"),
...
int CMarkup::x_FindElem(int iPosParent, int iPos, CString sz) const
{
    // If szPath is NULL or empty, go to next sibling element
    // Otherwise go to next sibling element with matching path

    LPCTSTR szPath = sz.c_str();
    if (iPos)
...
#define FINDNODEBAD(e) { szFindEnd=_T(">"); nEndLen=1; node.strMeta=FormatString(&node.strMeta,_T("Incorrect %s at offset %d"),e,nR); nNodeType=-1; }
    ...
    while (1)
    {
        ...
        if (! cD)
        {
            ...
            if (nNodeType != MNT_WHITESPACE && nNodeType != MNT_TEXT)
            {
                ...
                if ((nParseFlags & PD_DOCTYPE) || nNodeType == MNT_DOCUMENT_TYPE)
                    ...
                    szType = _T("Comment");
                nNodeType = -1;
                node.strMeta = FormatString(&node.strMeta, _T("%s at offset %d unterminated"), szType, node.nStart);
            }
            break;
        }
        ...
        if (nName)
        {
            if (_tcschr(_T(" \t\n\r/>"), (_TCHAR)cD))
            {
                ...
                {
                    char *copy_data;
                    copy_data = GetBuffer(node.strMeta, nNameLen);
                    memcpy(copy_data, &token.szDoc[nName], nNameLen * sizeof(_TCHAR));
                    memset(copy_data + nNameLen, 0, 1);
                    CString result(copy_data);
                    node.strMeta.swap(result);
                    free(copy_data);
                }
                nName = 0;
                cDminus2 = 0;
...
bool CMarkup::x_SetData(int iPos, LPCTSTR szData, int nFlags)
{
    ...
    if (iPos == m_iPos && m_nNodeLength)
    {
        ...
        x_AdjustForNode(m_iPosParent, iPos, strInsert.length() - m_nNodeLength);
        m_nNodeLength = strInsert.length();
        MARKUP_SETDEBUGSTATE;
        return true;
    }
    ...
    int nAdjust = node.strMeta.length() - nReplace;
    x_Adjust(iPos, nAdjust);
    m_aPos[iPos].nLength += nAdjust;
...
CString CMarkup::x_GetData(int iPos) const
{
    if (iPos == m_iPos && m_nNodeLength)
    {
        if (m_nNodeType == MNT_COMMENT)
            ...
            return Mid(m_strDoc, m_nNodeOffset + 4, m_nNodeLength - 7);
        else if (m_nNodeType == MNT_PROCESSING_INSTRUCTION)
            return Mid(m_strDoc, m_nNodeOffset + 2, m_nNodeLength - 4);
        else if (m_nNodeType == MNT_CDATA_SECTION)
            return Mid(m_strDoc, m_nNodeOffset + 9, m_nNodeLength - 12);
        else if (m_nNodeType == MNT_TEXT)
            return UnescapeText(&((LPCTSTR)m_strDoc.c_str())[m_nNodeOffset], m_nNodeLength);
        else if (m_nNodeType == MNT_LONE_END_TAG)
            return Mid(m_strDoc, m_nNodeOffset + 2, m_nNodeLength - 3);
        else
            return Mid(m_strDoc, m_nNodeOffset, m_nNodeLength);
    }
        ...
        if (pszTag && ((int)(pszTag - pszContent) < nContentLen))
        {
            ...
            while (token.nNext < nStartContent + nContentLen)
            {
                ...
                if (node.nNodeType == MNT_TEXT)
                    ...
                    strData += Mid(m_strDoc, node.nStart + 9, node.nLength - 12);
            }
        }
        else // no tags
            strData = UnescapeText(&((LPCTSTR)m_strDoc.c_str())[nStartContent], nContentLen);
    }
    return strData;
}
...
CString CMarkup::x_GetElemContent(int iPos) const
{
    ...
    */
    if (iPos && m_aPos[iPos].ContentLen())
    {
        CString csRet = Mid(m_strDoc, m_aPos[iPos].StartContent(), m_aPos[iPos].ContentLen());
        return UnescapeText(csRet , csRet.length());
    }
    return _T("");
}
...
bool CMarkup::x_SetElemContent(LPCTSTR szContent)
{
    ...
    CString csSContent = EscapeText(szContent, MNF_ESCAPEQUOTES);
    TokenPos token(csSContent, m_nFlags);
    ...
    NodePos node(MNF_WITHNOLINES | MNF_REPLACE);
    node.strMeta = csSContent;//szContent;
    int iPosBefore = 0;
    int nReplace = x_InsertNew(iPos, iPosBefore, node);
...
void CMarkup::x_DocChange(int nLeft, int nReplace, const CString &strInsert)
{
    ...
    _TCHAR *pDoc = GetBuffer(m_strDoc, nBufferLen);
    ...
    memcpy(&pDoc[nLeft], strInsert.c_str(), nInsLength * sizeof(_TCHAR));
...
int CMarkup::x_InsertNew(int iPosParent, int &iPosRel, CMarkup::NodePos &node)
{
    ...
    node.nLength = node.strMeta.length();
    ...
    if (bEmptyParentTag)
    {
        ...
        m_aPos[iPosParent].SetEndTagLen(3 + strTagName.length());
    }
    else
    {
...
bool CMarkup::x_AddElem(LPCTSTR szName, LPCTSTR szValue, int nFlags)
{
    ...
    x_Adjust(iPos, node.strMeta.length() - nReplace);
...
CString CMarkup::x_GetSubDoc(int iPos) const
{
    if (iPos)
    {
        ...
        if (! x_FindAny(szDoc, nChar) || szDoc[nChar] == _T('<'))
            nNext = nChar;
        return Mid(m_strDoc, nStart, nNext - nStart);
    }
    return _T("");
}
...
bool CMarkup::x_AddSubDoc(LPCTSTR szSubDoc, int nFlags)
{
    ...
    if (bWellFormed)
    {
        ...
        memcpy(GetBuffer(node.strMeta, nExtractLength + x_EOLLEN), &szSubDoc[nExtractStart], nExtractLength * sizeof(_TCHAR));
    ...
    x_Adjust(iPosLast, node.strMeta.length() - nReplace, true);
...
bool CMarkup::x_AddNode(int nNodeType, LPCTSTR szText, int nFlags)
{
    ...
    x_AdjustForNode(iPosParent, iPos, node.strMeta.length() - nReplace);
