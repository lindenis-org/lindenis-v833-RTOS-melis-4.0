...
GetPrivateProfileInt(LPCSTR lpAppName, LPCSTR lpKeyName, INT nDefault, LPCSTR lpFileName)
{
    ...
    DWORD nRet = GetPrivateProfileString(lpAppName, lpKeyName,
                                         "", lpReturnedString,
                                         sizeof(lpReturnedString), lpFileName);
    if (nRet == 0)
        return nDefault;
...
DWORD
GetPrivateProfileSectionNames(LPSTR lpszReturnBuffer, DWORD nSize, LPCSTR lpFileName)
{
    FILE *fp;
    static char szLine[10240] = {0};
    static char tmpstr[10240] = {0};
...
{
    FILE *fp;
    char lpKeyName[1024] = {0};
    strcpy(lpKeyName, lpAppName);
    ...
    while (!feof(fp))
    {
        ...
        if (rtnval == '\n')
        {
            ...
            if (endFlag == 1)
            {
                ...
                fwrite("\n", sizeof(char), 1, fp);
                fwrite("[", sizeof(char), 1, fp);
                fwrite(lpAppName, strlen(lpKeyName), 1, fp);
                fwrite("]", sizeof(char), 1, fp);
                fwrite("\n", sizeof(char), 1, fp);
                fwrite(lpString, strlen(lpString), 1, fp);
                fwrite("\n", sizeof(char), 1, fp);
