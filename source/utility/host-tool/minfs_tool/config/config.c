...
u32    GetKeyString(HCONFIG hcfg, s8 *szSection, s8 *szKey, s8 *String)
{
    ...
    pconfig = (CONFIG *) hcfg;
    dwRet = GetPrivateProfileString((const char *)szSection,
                                    (const char *)szKey,
                                    RAND_STRING,
                                    lpValue,
...
u32 ConfigDump(HCONFIG hcfg)
{
    ...
    if (OK != ret)
    {
        printf("ConfigDump,%s,%d,GetSectionCount failed ! %d", __FILE__, __LINE__, ret);
        return __LINE__;
    }
    //Msg("\nSectionCount=%d\n", SectionCount);
    ...
    if (NULL == pos)
    {
        printf("ConfigDump,%s,%d,%s", __FILE__, __LINE__, "GetFirstSection failed !");
        return __LINE__;
    }

    u32 i;
    for (i = 0; i < SectionCount && pos != NULL; i++)
    {
        memset(SectionName, 0, MAX_SECTION);
        pos = GetNextSection(hcfg, pos, (s8 *)SectionName);
        if (pos == NULL)
        {
            printf("ConfigDump,%s, %d, GetNextSection(%d) failed !", __FILE__, __LINE__, i);
            return __LINE__;
        }
        Msg("\n%d[%s]\n", i, SectionName);
        ...
        if (OK != ret)
        {
            printf("ConfigDump, %s, %d, GetSectionKeyCount(%d) failed !", __FILE__, __LINE__, ret);
            return __LINE__;
        }
        Msg("KeyCount=%d\n", KeyCount);
