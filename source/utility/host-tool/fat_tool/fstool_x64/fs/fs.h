...
typedef     int                 __le32;
//typedef       __int64             __le64;
typedef         long long int           __le64;     //modify by gongxf
typedef     unsigned short int  le16;
typedef     unsigned int        le32;
//typedef       unsigned __int64    le64;
typedef         unsigned long long int  le64;
...
struct inode
{
    ...
    struct msdos_inode_info *debug_fat_inode;
#endif
...
struct kstatfs
{
    ...
    __s32 f_charsettype;        /* fs charset type */
...
{
    int (*llseek)(struct file *, __s64, int);
    int (*read)(struct file *, char *, unsigned int, __s64 *);
    int (*write)(struct file *, const char *, unsigned int, __s64 *);
    int (*readdir)(struct file *, void *, filldir_t);
    int (*ioctl)(struct inode *, struct file *, unsigned int, unsigned int, void *);
...
static __inline void mark_inode_dirty(struct inode *inode)
{
    __mark_inode_dirty(inode, I_DIRTY);
}

static __inline void mark_inode_dirty_sync(struct inode *inode)
{
 ...
 */
static __inline void inc_nlink(struct inode *inode)
{
    inode->i_nlink++;
}

static __inline void inode_inc_link_count(struct inode *inode)
{
    inc_nlink(inode);
 ...
 * on the filesystem.
 */
static __inline void drop_nlink(struct inode *inode)
{
    inode->i_nlink--;
}
 ...
 */
static __inline void clear_nlink(struct inode *inode)
{
    inode->i_nlink = 0;
}

static __inline void inode_dec_link_count(struct inode *inode)
{
    drop_nlink(inode);
    mark_inode_dirty(inode);
}
...
static __inline void file_accessed(struct file *file)
{
...
extern struct super_block *path_to_sb(const char *pFullName, char **pFileName);
...
//
//static __inline struct inode *iget(struct super_block *sb, unsigned long ino)
//{
...
static __inline void insert_inode_hash(struct inode *inode)
{
...
extern int no_llseek(struct file *file, __s64 offset, int origin);
extern int generic_file_llseek(struct file *file, __s64 offset, int origin);
...
extern struct ktimestamp *fs_timespec_to_kstamp(struct ktimestamp *dest,
        struct timespec ts);
