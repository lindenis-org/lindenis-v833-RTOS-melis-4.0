...
static __inline unsigned long left_free_pages(unsigned int order)
{
...
{
    return &mem_map[(addr - (int)pagepool) >> PAGE_SHIFT];
}
...
int page_pool_ini(void)
{
    ...
    if ((int)__pagepool & ((1 << PAGE_SHIFT) - 1))
        pagepool = (char *)(((__u64)__pagepool + ((1 << PAGE_SHIFT) - 1)) & ~((1 << PAGE_SHIFT) - 1));
#else

    pagepool = palloc((NR_TOTAL_PAGE) << PAGE_SHIFT);
#endif
...
free_out:
    if (pagepool != NULL)
        pfree(pagepool, (NR_TOTAL_PAGE) << PAGE_SHIFT);
    for (i = 0; i < MAX_ORDER; i++)
    {
        if (mem_map_info.free_area[i].map)
...
/*----------------------epdk kswap thread -------------------*/
#define          KSWAP_Q_SIZE     5
__u8             kswap_tid;
__krnl_event_t  *kswapd_wait;

void wake_up_kswap(void)
{
#ifndef    simulate
    esKRNL_QPost(kswapd_wait, NULL);
#endif
}

int kswapd(void *parg)
{
    __u8 err;
    for (;;)
    {
        if (mem_map_info.free_pages > mem_map_info.pages_high)
            esKRNL_QPend(kswapd_wait, 0, &err);

        fs_log_trace01("kswapd() activated...\n");

        if (!try_to_free_pages())
        {
            fs_log_trace01("kswapd() sleeping...\n");
            esKRNL_TimeDly(3);
            continue;
        }
    }
...
int kswap_init(void)
{
    kswapd_wait = esKRNL_QCreate(KSWAP_Q_SIZE);
    if (kswapd_wait == (__krnl_event_t *)0)
    {
        fs_log_critical("create kswap queue fail!");
        return -1;
    }

    kswap_tid = esKRNL_TCreate(kswapd, 0, 0x400, (EPOS_curid << 8) | KRNL_priolevel6);

    fs_log_verbose("Kswap thread is created, priority:6.\n");
    return 0;
}
