...
static int p_hash(unsigned int perent_hash,
                  unsigned int hash)
{
...
struct dentry *d_alloc(struct dentry *parent, const struct qstr *name)
{
    ...
    {
        dentry->d_pphash_ = (int)dentry + esKSRV_Random(0xffffffff);
    }
    dentry->d_phash_ = p_hash(dentry->d_pphash_, name->hash);
...
int vfs_caches_init(unsigned long mempages)
{
    ...
    if (!names_cachep)
        fs_log_critical("Cannot create names SLAB cache");
