...
struct mutex {};
struct semaphore {};
void __inline init_MUTEX(struct mutex *lock)
{
}
#define up(x)
#define down(x)
...
//  init_MUTEX(&cache_chain_sem);
    ...
    /* initialize cache_cache struct first,
     * By Sunny at 2010-5-27-9-58
     */
    INIT_LIST_HEAD(&(cache_cache.slabs_full));
    INIT_LIST_HEAD(&(cache_cache.slabs_partial));
    INIT_LIST_HEAD(&(cache_cache.slabs_free));
 ...
 */
static __inline void *kmem_getpages(kmem_cache_t *cachep, unsigned long flags)
{
    void    *addr;
...
static __inline void kmem_freepages(kmem_cache_t *cachep, void *addr)
{
    unsigned long i = (1 << cachep->gfporder);
    //struct page *page = virt_to_page((__u64)addr);
    struct page *page = virt_to_page((int)addr);
...
static void kmem_slab_destroy(kmem_cache_t *cachep, slab_t *slabp)
{
    if (cachep->dtor)
    {
        ...
        {
            void *objp = (void *)((unsigned int)slabp->s_mem + cachep->objsize * i);
            if (cachep->dtor)
                (cachep->dtor)(objp, cachep, 0);
        }
    }

    kmem_freepages(cachep, (void *)((unsigned int)slabp->s_mem - slabp->colouroff));
    if (OFF_SLAB(cachep))
 ...
 */
kmem_cache_t *
kmem_cache_create(const char *name, unsigned int size, unsigned int offset,
...
static __inline slab_t *kmem_cache_slabmgmt(kmem_cache_t *cachep,
        void *objp, int colour_off, int local_flags)
{
    ...
    else
    {
         ...
         */
        slabp = (void *)((__u64)objp + colour_off);
        printf("slabp:0x%llx\n", slabp);
    ...
    slabp->s_mem = (void *)((__u64)objp + colour_off);

    return slabp;
}

static __inline void kmem_cache_init_objs(kmem_cache_t *cachep,
        slab_t *slabp, unsigned long ctor_flags)
{
    ...
    for (i = 0; i < cachep->num; i++)
    {
        void *objp = (void *)((__u64)slabp->s_mem + cachep->objsize * i);
...
static int kmem_cache_grow(kmem_cache_t *cachep, int flags)
{
    ...
    i = 1 << cachep->gfporder;
    //page = virt_to_page((__u64)objp);
    page = virt_to_page((int)objp);
    do
    {
...
static __inline void *kmem_cache_alloc_one_tail(kmem_cache_t *cachep,
        slab_t *slabp)
{
    void *objp;
    ...
    /* get obj pointer */
    slabp->inuse++;
    objp = (void *)((__u64)slabp->s_mem + slabp->free * cachep->objsize);
...
void kmem_cache_free(kmem_cache_t *cachep, void *objp)
{
     ...
     */
    //slabp = GET_PAGE_SLAB(virt_to_page((__u64)objp));
    slabp = GET_PAGE_SLAB(virt_to_page((int)objp));

    {
        //__u64 objnr = ((__u64)objp-(__u64)slabp->s_mem)/cachep->objsize;
        unsigned int objnr = ((unsigned int)objp - (unsigned int)slabp->s_mem) / cachep->objsize;
...
void kfree(const void *objp)
{
    ...
    if (!objp)
        return;
//  c = GET_PAGE_CACHE(virt_to_page((__u64)objp));
    c = GET_PAGE_CACHE(virt_to_page((int)objp));
...
kmem_cache_t *kmem_find_general_cachep(unsigned int size, int gfpflags)
{
     ...
     * made __inline so consumers can quickly determine what
