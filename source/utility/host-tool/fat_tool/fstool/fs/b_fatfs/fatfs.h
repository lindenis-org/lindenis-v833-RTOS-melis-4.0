...
static __inline struct msdos_sb_info *MSDOS_SB(struct super_block *sb)
{
    return sb->s_fs_info;
}

static __inline struct msdos_inode_info *MSDOS_I(struct inode *inode)
{
    return container_of(inode, struct msdos_inode_info, vfs_inode);
}

/* Return the FAT attribute byte for this inode */
static __inline __u8 fat_attr(struct inode *inode)
{
...
static __inline unsigned char fat_checksum(const __u8 *name)
{
...
static __inline int fat_clus_to_blknr(struct msdos_sb_info *sbi, int clus)
{
    return ((int)clus - FAT_START_ENT) * sbi->sec_per_clus
           + sbi->data_start;
}

//static __inline void fat16_towchar(wchar_t *dst, const __u8 *src, unsigned int len)
//{
//#ifdef __BIG_ENDIAN
//  while (len--) {
//      *dst++ = src[0] | (src[1] << 8);
...
//}
//
//static __inline void fatwchar_to16(__u8 *dst, const wchar_t *src, unsigned int len)
//{
//#ifdef __BIG_ENDIAN
//  while (len--) {
//      dst[0] = *src & 0x00FF;
...
void fat16_towchar(wchar_t *dst, const __u8 *src, unsigned int len);
void fatwchar_to16(__u8 *dst, const wchar_t *src, unsigned int len);
...
static __inline void fatent_init(struct fat_entry *fatent)
{
    fatent->nr_bhs = 0;
    fatent->entry = 0;
...
static __inline void fatent_set_entry(struct fat_entry *fatent, int entry)
{
    fatent->entry = entry;
    fatent->u.ent32_p = NULL;
}

static __inline void fatent_brelse(struct fat_entry *fatent)
{
    int i;
    fatent->u.ent32_p = NULL;
