                    ...
                    int *phys, int create)
{
    ...
    nr_newclus = ALIGN((*max_blocks - sec_in_alloced_clus), sbi->sec_per_clus)
                 >> sec_per_clus_bits;
    if (nr_newclus)
    {
...
//  sb_min_blocksize(sb, 512);
    ...
    if (logical_sector_size > sb->s_blocksize)
    {
        ...
        sb->s_blocksize_bits = ({unsigned int bits = 8, __size = logical_sector_size;
        do {bits++; __size >>= 1;}
        while (__size > 256);
        bits;
                                });
...
//      }
    ...
    sbi->root_cluster = 0;
//modify by gongxf
//  (int)sbi->free_clusters = -1;   /* Don't know yet */
    sbi->free_clusters = -1;
    ...
    eLIBs_int2str_dec(sbi->fat_bits,
                      &sb->s_fsname[eLIBs_strlen(sb->s_fsname)]);
...
//modify by gongxf
//  if ((int)sbi->free_clusters != -1 && sbi->free_clusters > total_clusters)
//      (int)sbi->free_clusters = -1;
