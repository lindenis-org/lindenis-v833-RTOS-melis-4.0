...
int fat_get_cluster(struct inode *inode, int cluster, int *fclus, int *dclus, int *mapped)
{
    ...
    *dclus = MSDOS_I(inode)->i_start;
    ...
    if (mapped) *mapped = cid.nr_contig - (cluster - cid.fcluster) + 1;
...
void debug_show_lru_fatcache(struct list_head *head, struct fat_cache_id *cid, char *name, __u32 fclus)
{
    ...
    if (head)
    {
        ...
        {
            __inf("lru = fclus:%d, dclus:%d, cont_nr:%d\n", p->fcluster, p->dcluster, p->nr_contig);
        }
    }
    if (cid)
        __inf("cid = fclus:%d, dclus:%d, cont_nr:%d\n", cid->fcluster, cid->dcluster, cid->nr_contig);
}
#endif
