...
__hdle esFSYS_opendir(const char *pDirName)
{
    ...
    if (err)
    {
        fs_log_warning("opendir err when pend sem, err:%d\n", err);
        return NULL;
    }
    ...
    /* 分配dirent空间 */
    retval->private_data = __get_free_page(0);// kmalloc(sizeof(__fsys_dirent_t), 0);
...
int esFSYS_mkdir(const char *pDirName)
{
    struct super_block  *sb;
    char *realname;
    int error = 0;
    ...
    if (err)
    {
        fs_log_warning("mkdir err when pend sem, err:%d\n", err);
        return EPDK_FAIL;
    }
...
int esFSYS_rmdir(const char *pDirName)
{
    struct super_block  *sb;
    char *realname;
    int error = 0;
    ...
    if (err)
    {
        fs_log_warning("rmdir err when pend sem, err:%d\n", err);
        return EPDK_FAIL;
    }
                   ...
                   __s64 offset, __u64 ino, unsigned int d_type, __u64 d_size)
{
    ...
    int reclen = ALIGN(NAME_OFFSET(dirent) + namlen + 1, sizeof(__u64));
    ...
    eLIBs_strncpy(dirent->d_name, name, namlen);
    buf->previous = dirent;
...
static __fsys_dirent_t *__getonedir(struct file *dir)
{
    ...
    do
    {
        if (dirbuf->offset >= dirbuf->size)
        {
            ...
            res = __getdents(dir, dirbuf->data, dirbuf->allocation, &bytes);
            if (res < 0)
    ...
    eLIBs_strncpy(out->d_name, dp->d_name, FSYS_DIRNAME_MAX);
...
__hdle esFSYS_readdir(__hdle hDir)
{
    ...
    if (err)
    {
        fs_log_warning("readdir err when pend sem, err:%d\n", err);
        return NULL;
    }
...
void esFSYS_rewinddir(__hdle hDir)
{
    ...
    if (err)
    {
        fs_log_warning("rewinddir err when pend sem, err:%d\n", err);
        return;
    }
