...
static int pagebufsinteresting(struct page *page)
{
    ...
    if (!___bh) return intresting;

    do
    {
        if (atomic_read(&___bh->b_count)) intresting = 1;
        if (buffer_dirty(___bh)) intresting = 1;
...
void debug_show_sb_inode(struct list_head *head, char *name, int align_level)
{
    ...
    for (i = 0; i < align_level; i++) space[i] = ' ';
    __inf("\n\n%s>>>>>%s", space, name);
    ...
    __inf("\n%s--result-- : %d item in upper \"%s\" list.\n", space, i, name);
}
...
BOOL fs_dump_parse_option(const char *option)
{
    ...
    else if (strcmp(option, "memseg") == 0 || strcmp(option, "MEMSEG") == 0)
    {
        ...
        __inf("%d segment memory dumps.\n", memd_cnt);
        for (i = 0; ma = &memd_array[i], i < memd_cnt; i++)
        {
            __inf("start: %d, length: %d file: %s\n", ma->start, ma->length, ma->name);
        }
        return TRUE;
    }
    ...
    __inf("Unknown fs dump option '%s'\n", option);
    return FALSE;
}
