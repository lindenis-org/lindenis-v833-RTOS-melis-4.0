...
static int mmu_check_valid_address(void *start_addr, void *end_addr)
{
    ...
    if (check_virtual_address((uint32_t)start_addr) != 1)
    {
        return 0;
    }
    ...
    if (end_addr != NULL)
    {
        if (check_virtual_address((uint32_t)end_addr) != 1)
        {
            return 0;
        }
    }
...
static int cmd_print_mem_value(int argc, const char **argv)
{
    ...
    if (argc > 2)
    {
        ...
        {
            printf("start_addr=0x%08x end_addr=0x%08x\n", (unsigned int)start_addr, (unsigned int)end_addr + 4);
            for (; start_addr <= end_addr;)
            {
                printf("[0x%08x]=0x%08x \n", (unsigned int)start_addr, *((volatile unsigned int *)(start_addr)));
                start_addr += 4;
            }
        }
        else
