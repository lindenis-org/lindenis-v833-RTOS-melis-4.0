		...
		if (SOCK_CMD_STAT_AE == type) {
			...
			for (i = 0; i < ISP_AF_ROW; i++) {
				for (j = 0; j < ISP_AF_COL; j++) {
					index += sprintf(&buf[index], "%06llu, ", (long unsigned int)af_info->af_count[i][j]);
				}
				index += sprintf(&buf[index], "\n");
			}
			// af_h_d1
			...
			for (i = 0; i < ISP_AF_ROW; i++) {
				for (j = 0; j < ISP_AF_COL; j++) {
					index += sprintf(&buf[index], "%06llu, ", (long unsigned int)af_info->af_h_d1[i][j]);
				}
				index += sprintf(&buf[index], "\n");
			}
			// af_h_d2
			...
			for (i = 0; i < ISP_AF_ROW; i++) {
				for (j = 0; j < ISP_AF_COL; j++) {
					index += sprintf(&buf[index], "%06llu, ", (long unsigned int)af_info->af_h_d2[i][j]);
				}
				index += sprintf(&buf[index], "\n");
			}
			// af_v_d1
			...
			for (i = 0; i < ISP_AF_ROW; i++) {
				for (j = 0; j < ISP_AF_COL; j++) {
					index += sprintf(&buf[index], "%06llu, ", (long unsigned int)af_info->af_v_d1[i][j]);
				}
				index += sprintf(&buf[index], "\n");
			}
			// af_v_d2
			...
			for (i = 0; i < ISP_AF_ROW; i++) {
				for (j = 0; j < ISP_AF_COL; j++) {
					index += sprintf(&buf[index], "%06llu, ", (long unsigned int)af_info->af_v_d2[i][j]);
				}
				index += sprintf(&buf[index], "\n");
			}
