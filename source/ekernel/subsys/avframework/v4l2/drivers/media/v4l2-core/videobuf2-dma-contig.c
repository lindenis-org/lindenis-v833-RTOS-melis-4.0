...
extern uint32_t awos_arch_virt_to_phys(uint32_t virtaddr);
#undef __va_to_pa
...
static void vb2_dc_prepare(void *buf_priv)
{
	...
	{
		cpu_dcache_invalidate((unsigned long)buf->cookie, buf->size);
	}
	else
	{
		cpu_dcache_clean((unsigned long)buf->cookie, buf->size);
	}
}
