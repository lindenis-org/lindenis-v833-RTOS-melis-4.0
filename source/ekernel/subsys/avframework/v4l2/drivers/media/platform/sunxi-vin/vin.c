...
static int vin_md_clk_enable(struct vin_md *vind)
{
	...
	if (vind->clk[VIN_TOP_CLK]->clock != HAL_CLK_UNINITIALIZED) {
		...
		sunxi_clk_periph_disable((clk_periph_pt)clk_get_core(vind->clk[VIN_TOP_CLK]->clock));
		sunxi_clk_periph_enable((clk_periph_pt)clk_get_core(vind->clk[VIN_TOP_CLK]->clock));
	}
	...
	if (vind->isp_clk[VIN_ISP_CLK]->clock != HAL_CLK_UNINITIALIZED) {
		...
		sunxi_clk_periph_disable((clk_periph_pt)clk_get_core(vind->isp_clk[VIN_ISP_CLK]->clock));
		sunxi_clk_periph_enable((clk_periph_pt)clk_get_core(vind->isp_clk[VIN_ISP_CLK]->clock));
	}
