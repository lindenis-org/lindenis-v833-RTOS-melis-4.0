...
void *minfs_allocate_temp_buffer(struct super_block *sb, __u32 len)
{
...
void minfs_free_temp_buffer(struct super_block *sb, void *buffer, __u32 len)
{
    pfree(buffer, len);
}
...
{
    struct minfs_sb_info *sbi = MINFS_SB(sb);
    __u32  sector = (pos >> MINFS_CACHE_BITS) << \
                    (MINFS_CACHE_BITS - sb->s_blocksize_bits);
    ...
    if (offset)
    {
        ...
        chunk_size = left <= (MINFS_CACHE_SIZE - offset) ? \
                     left : (MINFS_CACHE_SIZE - offset);
    ...
    if (left >> MINFS_CACHE_BITS)
    {
        ...
        direct_nr = (left >> MINFS_CACHE_BITS) << \
                    (MINFS_CACHE_BITS - sb->s_blocksize_bits);
