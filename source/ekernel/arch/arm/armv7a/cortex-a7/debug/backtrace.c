...
static int find_lr_offset(char *LR, print_function print_func)
{
    ...
    if (LR_fixed == PC2ADDR(&rt_thread_exit))
    {
        if (print_func != NULL)
        {
            long2str((long)&rt_thread_exit, &backtrace_output_buf[14]);
            print_func(backtrace_output_buf);
        }
        return 0;
    }
    ...
    if (IS_THUMB_ADDR(LR))
    {
        ...
        if (IS_VALID_TEXT_ADDRESS(LR_fixed - 4))
        {
                                   ...
                                   print_function print_func)
{
    ...
    for (i = 2; i < BT_SCAN_MAX_LIMIT; i += 2)
    {
        if (IS_VALID_TEXT_ADDRESS(PC - i) == 0)
        {
            if (print_func != NULL)
            {
                print_func("backtrace : invalid lr\r\n");
        ...
        if ((thumb_ins & 0xff80) == 0xb080)
        {
            ...
            parse_addr  = PC - i;
            if (IS_VALID_TEXT_ADDRESS(PC - i - 2) == 0)
            {
                if (print_func != NULL)
                {
                    print_func("backtrace : invalid lr\r\n");
