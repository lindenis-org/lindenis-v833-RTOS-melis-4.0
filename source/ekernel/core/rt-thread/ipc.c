...
rt_err_t rt_mutex_release(rt_mutex_t mutex)
{
    ...
    if (mutex->hold == 0)
    {
        ...
        if (!rt_list_isempty(&mutex->parent.suspend_thread))
        {
            ...
            if ((mutex->parent.parent.flag == RT_IPC_FLAG_FIFO) && (thread->current_priority != max_priority_in_queue))
            {
                rt_thread_control(thread, RT_THREAD_CTRL_CHANGE_PRIORITY, &(max_priority_in_queue));
            }
