    ...
    if (dir->lu_cache[0] != NULL) {
        ...
        if (dir->top == NULL || find_empty_slot(dir->top, &id_index) == NULL) {
            if (dir->top && dir->cur_max_level > 2) {
                printf("a full idr,we only support 32bit\n");
                goto out;
            }
            tmp = (struct id_layer*)malloc(sizeof(struct id_layer));
        ...
        for (cur= &dir->top; *cur != NULL; id <<= 8, id += id_index, curr_lvl--) {
            ...
            if (curr_lvl > 0 && *cur == NULL) {
                //add a new child idr
                *cur = (struct id_layer*)malloc(sizeof(struct id_layer));
                if (*cur == NULL) {
                    printf("atomic alloc idr mem err.\n");
                    goto out;
                }
...
struct id_dir* id_creat(void)
{
    struct id_dir* idr;
    int i;
    idr = (struct id_dir*) malloc(sizeof(struct id_dir));
    if(idr == NULL) {
        printf("malloc idr err.\n");
        return NULL;
    }
    memset(idr, 0, sizeof(struct id_dir));
    idr->cur_max_level = -1;
    hal_sem_create(&idr->lock, 1);
    for(i = 0; i < 4; i++)
        idr->lu_cache[i] = NULL;
    idr->lu_id = 0;
    idr->head = NULL;
    idr->top = (struct id_layer*) malloc(sizeof(struct id_layer));
    if (idr->top == NULL) {
        printf("malloc id_layer err.\n");
        goto out;
    }
