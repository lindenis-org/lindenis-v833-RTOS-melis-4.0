...
static void list_add_tail(struct g2d_mixer_task *node, struct g2d_mixer_task *head)
{
...
static void list_del(struct g2d_mixer_task *node)
{
	struct g2d_mixer_task *temp = g2d_task_list;
				 ...
				 struct mixer_para *p_para)
{
	...
	p_task->p_rcq_info->reg_blk =
	    malloc(sizeof(*(p_task->p_rcq_info->reg_blk)) *
			p_task->p_rcq_info->cur_num);
			   ...
			   struct mixer_para *p_para)
{
	...
	cpu_dcache_clean((unsigned long)p_task->p_rcq_info->vir_addr,p_task->p_rcq_info->rcq_reg_mem_size);
			 ...
			 unsigned int frame_len)
{
	...
	task = malloc(sizeof(struct g2d_mixer_task));
	if (!task) {
	...
	task->frame = malloc(frame_len*sizeof(struct g2d_mixer_frame));

	task->p_rcq_info =
	    malloc(sizeof(struct g2d_rcq_mem_info));
	...
	for (i = 0; i < frame_len; ++i) {
		...
		task->frame[i].g2d_base = (u8 *)p_g2d_info->io;
	}
	...
	task->p_para = malloc(sizeof(struct mixer_para) * task->frame_cnt);
	if (!task->p_para) {
		task->destory(task);
		goto OUT;
	}
	...
	list_add_tail(task, g2d_task_list);
