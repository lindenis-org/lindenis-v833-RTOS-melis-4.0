...
typedef struct {
	rt_uint32_t addr;
	rt_uint32_t len; /* in word (4 bytes). Exception: in byte for AES_CTS */
} ce_scatter_t;

typedef struct ce_task_desc {
	rt_uint32_t chan_id;
	rt_uint32_t comm_ctl;
	rt_uint32_t sym_ctl;
	rt_uint32_t asym_ctl;

	rt_uint32_t key_addr;
	rt_uint32_t iv_addr;
	rt_uint32_t ctr_addr;
	rt_uint32_t data_len; /* in word(4 byte). Exception: in byte for AES_CTS */
