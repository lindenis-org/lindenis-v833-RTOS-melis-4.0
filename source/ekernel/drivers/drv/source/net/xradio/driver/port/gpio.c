...
void xr_wlan_irq_subscribe(void *func)
{
	...
	hal_gpio_set_pull(GPIO_PG6, GPIO_PULL_UP);
	hal_gpio_set_direction(GPIO_PG6, GPIO_DIRECTION_INPUT);	/*pin_x mode */
	hal_gpio_set_driving_level(GPIO_PG6, GPIO_DRIVING_LEVEL2);
	hal_gpio_to_irq(GPIO_PG6, &irq_num);
...
{
	if (state)
		hal_gpio_set_pull(GPIO_PG7, GPIO_PULL_UP);
	else
		hal_gpio_set_pull(GPIO_PG7, GPIO_PULL_DOWN);

	hal_gpio_set_direction(GPIO_PG7, GPIO_DIRECTION_OUTPUT);
	hal_gpio_set_driving_level(GPIO_PG7, GPIO_DRIVING_LEVEL2);
}
...
int xradio_wlan_gpio_power(int on)
{
	...
	if (on) {
		//enable 24M clk fout.
		*((volatile uint32_t *)(0x07000160)) = 0x083f10f7;

		hal_gpio_set_data(GPIO_PG7, GPIO_DATA_HIGH);
		XR_OS_MSleep(10);
		hal_gpio_set_data(GPIO_PG7, GPIO_DATA_LOW);
		XR_OS_MSleep(20);
		hal_gpio_set_data(GPIO_PG7, GPIO_DATA_HIGH);
		XR_OS_MSleep(500);
	} else {
		hal_gpio_set_data(GPIO_PG7, GPIO_DATA_LOW);
		XR_OS_MSleep(50);
		//disable 24M clk fout.
		*((volatile uint32_t *)(0x07000160)) = 0x883f10f7;
	}
