...
void bcm_wifi_start(unsigned char country[2]) {
  int ret;

  mhd_set_country_code(MHD_MK_CNTRY( country[0], country[1], 0 ));
...
int mhd_tcpka_config( char *server_ip, uint16_t server_port, uint16_t interval, uint16_t retry_interval, uint16_t retry_count, char *ka_payload, int ka_payload_len );
...
void bcm_tcpka_commands( int argc, char **argv )
{
	...
	if (strcmp((const char*)argv[0], "add") == 0) {
	    ...
	    {
	        ret = mhd_tcpka_config(KA_STA_SERVER_IP, KA_STA_SERVER_PORT, KA_STA_INTERVAL, 2, 4, ka_payload, ka_payload_len);
	    }
	    else if (argc == (bcm_cmd_argc+1))
	    {
	        char *server_ip = argv[bcm_index_of_arg];
	        ret = mhd_tcpka_config(server_ip, KA_STA_SERVER_PORT, KA_STA_INTERVAL, 2, 4, ka_payload, ka_payload_len);
	    }
	    else if (argc == (bcm_cmd_argc+2))
	    {
	        char *server_ip = argv[bcm_index_of_arg];
	        int server_port = atoi(argv[bcm_index_of_arg+1]);
	        ret = mhd_tcpka_config(server_ip, server_port, KA_STA_INTERVAL, 2, 4, ka_payload, ka_payload_len);
	    }
	    else if (argc == (bcm_cmd_argc+3))
	    {
	        ...
	        ret = mhd_tcpka_config(server_ip, server_port, interval, 2, 4, ka_payload, ka_payload_len);
	    }
...
{
	FILE* fp;
	struct mmc_card_info *write_card_info;
	struct mmc_card_info read_card_info;

	fp = fopen(sdio_cardinfo_file, "a+");
	if (fp == NULL) {
		printf("%s open failed\n", sdio_cardinfo_file);
		return;
	}

	memset(&read_card_info, 0, sizeof(struct mmc_card_info));

	fseek(fp, 0L, SEEK_SET);
	fread(&read_card_info, sizeof(struct mmc_card_info), 1, fp);

	write_card_info = mmc_card_save(card_id);
	if (!write_card_info)
		goto fail;

	if (!memcmp(&read_card_info, write_card_info, sizeof(struct mmc_card_info)))
	{
		free(write_card_info);
		fclose(fp);
		return;
	}

	printk("different sdio card information, need to update!\n");
	fwrite(write_card_info, sizeof(struct mmc_card_info), 1, fp);
