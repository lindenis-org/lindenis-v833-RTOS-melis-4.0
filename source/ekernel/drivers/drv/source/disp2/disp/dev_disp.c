...
struct rt_workqueue *g_disp_work_queue;
...
static void start_work(struct rt_work *work, void* work_data)
{
	int num_screens;
	int screen_id;
	int count = 0;
...
{
	rt_workqueue_dowork(g_disp_work_queue, &g_disp_drv.start_work);
	return 0;
}
...
static rt_err_t disp_init(void)
{
	...
	g_disp_work_queue = rt_workqueue_create("disp2", 8192, 15);
	...
	rt_work_init(&g_disp_drv.start_work, start_work, NULL);
	...
	for (i = 0; i < DISP_MOD_NUM; i++) {
		...
		DE_INF("mod %d, base=0x%lx, irq=%d, mclk=0x%p\n", i,
...
static rt_err_t disp_probe(struct rt_device *dev)
{
	int i;
	rt_err_t ret;
	int counter = 0;
	...
	struct sunxi_display_engine *engine = (struct sunxi_display_engine *)dev;

	if(engine->status)
	{
		DE_INF("display engine already opended.\n");
		return 0;
	}
...
static rt_err_t disp_ioctl(rt_device_t dev, int cmd, void *arg)
{
...
rt_err_t disp_release(rt_device_t dev)
{
#if 0
...
rt_err_t disp_open(rt_device_t dev, rt_uint16_t oflag)
{
	return 0;
}

struct sunxi_display_engine disp_engine = {
    .disp_engine = {
	.init = disp_probe,
	.open = disp_open,
	.close = disp_release,
...
int disp_module_init(void)
{
	...
	ret = rt_device_register(&disp_engine.disp_engine, "disp", RT_DEVICE_FLAG_RDWR);
	if (ret) {
		DE_WRN("Faile to register disp device");
		goto OUT;
	}

	ret = disp_probe(&disp_engine.disp_engine);
	disp_engine.status = 1;
OUT:
	return ret;
}
