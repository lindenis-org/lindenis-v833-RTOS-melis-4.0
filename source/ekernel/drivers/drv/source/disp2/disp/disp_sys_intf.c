...
s32 disp_sys_clk_set_rate(void *p_clk, u32 rate)
{
	hal_clk_status_t ret;
	hal_clk_id_t clk = (hal_clk_id_t)p_clk;

	ret = hal_clk_set_rate(clk, rate);
...
u32 disp_sys_clk_get_rate(void *p_clk)
{
	hal_clk_id_t clk = (hal_clk_id_t)p_clk;
...
s32 disp_sys_clk_enable(void *clk)
{
	hal_clk_status_t ret;
	hal_clk_id_t parent;
	...
	if (!plat_get_clk_parent(child, &parent)) {
		ret = hal_clock_enable(parent);
		if (ret) {
			DE_INF("Enable parent clk:%d fail:%d\n", parent, ret);
		}
	}

	ret = hal_clock_enable(child);
	if (ret) {
...
s32 disp_sys_clk_disable(void *clk)
{
	hal_clk_status_t ret;
	hal_clk_id_t parent;
	hal_clk_id_t child = (hal_clk_id_t)clk;

	ret = hal_clock_disable(child);
	if (ret) {
		DE_WRN("Enable clk:%d fail:%d\n", child, ret);
		return -1;
	}
...
s32 disp_sys_clk_set_parent(void *clk, void *parent)
{
	hal_clk_status_t ret;
	hal_clk_id_t child = (hal_clk_id_t)clk;
	hal_clk_id_t papa = (hal_clk_id_t)parent;

	ret = hal_clk_set_parent(child, papa); 
	if (!ret)
		return 0;
	else {
...
void* disp_sys_clk_get_parent(void *clk)
{
	hal_clk_id_t child = (hal_clk_id_t)clk;
	hal_clk_id_t papa;
...
#ifdef CONFIG_OF
#else
	hal_clk_id_t clk_no = 0;
	if(plat_get_clk(index, &clk_no))
		DE_WRN("Get clk no %d fail!\n", index);
	return clk_no;
#endif
	return 0;
}
