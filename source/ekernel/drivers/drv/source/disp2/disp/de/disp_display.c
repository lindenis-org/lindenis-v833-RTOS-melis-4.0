...
static void disp_tasklet(struct rt_work *work, void* work_data);
...
s32 bsp_disp_init(struct disp_bsp_init_para *para)
{
	...
	for (disp = 0; disp < num_screens; disp++) {
		...
		rt_work_init(&gdisp.screen[disp].tasklet, disp_tasklet, (void *)disp);
	}
...
s32 bsp_disp_shadow_protect(u32 disp, bool protect)
{
	...
	if (protect) {
		while ((ret != 0) && (cnt < max_cnt)) {
			gdisp.screen[disp].flag_lock = hal_spin_lock_irqsave();
			cnt++;
			if (gdisp.screen[disp].have_cfg_reg == false) {
				gdisp.screen[disp].cfg_cnt++;
				ret = 0;
			}
			hal_spin_unlock_irqrestore(
		...
		if (ret != 0) {
			...
			gdisp.screen[disp].flag_lock = hal_spin_lock_irqsave();
			gdisp.screen[disp].cfg_cnt++;
			hal_spin_unlock_irqrestore(gdisp.screen[disp].flag_lock
					       );
		}
	} else {
		gdisp.screen[disp].flag_lock = hal_spin_lock_irqsave();
...
extern struct rt_workqueue *g_disp_work_queue;
void sync_event_proc(u32 disp, bool timeout)
{
	...
	rt_workqueue_dowork(g_disp_work_queue, &gdisp.screen[disp].tasklet);
}
