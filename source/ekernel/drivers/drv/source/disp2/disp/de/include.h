...
#define DE_INF(...) do { \
			LOG_COLOR("[DISP]", SYS_LOG_COLOR_YELLOW, ##__VA_ARGS__); \
		} while (0)
#define DE_MSG(...) do { \
			LOG_COLOR("[DISP]", SYS_LOG_COLOR_OFF, ##__VA_ARGS__); \
		} while (0)
#define DE_HERE
#define DE_DBG(msg...)
...
#define DE_HERE do { \
			LOG_COLOR("[DISP]", SYS_LOG_COLOR_OFF); \
		} while (0)
#define DE_DBG(...) do { \
			LOG_COLOR("[DISP]", SYS_LOG_COLOR_OFF, ##__VA_ARGS__); \
		} while (0)
...
#define DE_WRN(...) do { \
			__log("[DISP]", SYS_LOG_COLOR_YELLOW, ##__VA_ARGS__); \
		} while (0)
...
struct disp_device {
	...
	struct rt_work close_eink_panel_work;
	void *priv_data;
