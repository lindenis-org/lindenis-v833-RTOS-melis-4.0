...
static s32 lcd_parse_panel_para(u32 disp, struct disp_panel_para *info)
{
	...
	sprintf(primary_key, "lcd%lu", disp);
...
static s32 lcd_clk_init(struct disp_device *lcd)
{
	...
	lcdp->clk_parent = (disp_clk_t)disp_sys_clk_get_parent((void *)lcdp->clk);
...
static s32 cal_real_frame_period(struct disp_device *lcd)
{
	...
	if (lcdp->panel_info.lcd_if == LCD_IF_DSI)
		lcd->timings.dclk_rate_set =
		    disp_sys_clk_get_rate((void *)lcdp->clk) / clk_info.dsi_div;
	else
		lcd->timings.dclk_rate_set =
		    disp_sys_clk_get_rate((void *)lcdp->clk) / clk_info.tcon_div;
...
static s32 lcd_clk_config(struct disp_device *lcd)
{
	...
	if (lcdp->clk_parent) {
		disp_sys_clk_set_rate((void *)lcdp->clk_parent, pll_rate);
		pll_rate_set = disp_sys_clk_get_rate((void *)lcdp->clk_parent);
		disp_sys_clk_set_parent((void *)lcdp->clk, (void *)lcdp->clk_parent);
	}
	...
	disp_sys_clk_set_rate((void *)lcdp->clk, lcd_rate_set);
	lcd_rate_set = disp_sys_clk_get_rate((void *)lcdp->clk);
	if (lcdp->panel_info.lcd_if == LCD_IF_DSI) {
		...
		for (i = dsi_num * lcd->disp, j = 0; i < CLK_DSI_NUM;
		     i += CLK_NUM_PER_DSI) {
			if (lcdp->dsi_clk[i]) {
				parent = (disp_clk_t)disp_sys_clk_get_parent((void *)lcdp->dsi_clk[i]);
				disp_sys_clk_set_parent((void *)lcdp->dsi_clk[i], (void *)parent);
				disp_sys_clk_set_rate((void *)lcdp->dsi_clk[i], dsi_rate_set);
				DE_INF(
...
static s32 lcd_clk_enable(struct disp_device *lcd)
{
	...
	if (hal_clock_is_enabled(lcdp->clk) != HAL_CLK_STATUS_ENABLED) {
		ret = disp_sys_clk_enable((void *)lcdp->clk);
		if (ret != 0) {
	...
	if (lcdp->panel_info.lcd_if == LCD_IF_LVDS) {
		if (lcdp->lvds_clk) {
			parent = (disp_clk_t)disp_sys_clk_get_parent((void *)lcdp->lvds_clk);
			disp_sys_clk_set_parent((void *)lcdp->lvds_clk, (void *)parent);
			ret = disp_sys_clk_enable((void *)lcdp->lvds_clk);
		}
		if (ret != 0) {
		...
		for (i = dsi_num * lcd->disp, j = 0; i < CLK_DSI_NUM; ++i) {
			if (lcdp->dsi_clk[i]) {
				ret = disp_sys_clk_enable((void *)lcdp->dsi_clk[i]);
				if (ret != 0) {
		...
		parent = (disp_clk_t)disp_sys_clk_get_parent((void *)lcdp->edp_clk);
		disp_sys_clk_set_parent((void *)lcdp->edp_clk, (void *)parent);
		ret = disp_sys_clk_enable((void *)lcdp->edp_clk);
		if (ret != 0) {
...
static s32 lcd_clk_disable(struct disp_device *lcd)
{
	...
	if (lcdp->panel_info.lcd_if == LCD_IF_LVDS) {
		disp_sys_clk_disable((void *)lcdp->lvds_clk);
	} else if (lcdp->panel_info.lcd_if == LCD_IF_DSI) {
		dsi_num = (lcdp->panel_info.lcd_tcon_mode == DISP_TCON_DUAL_DSI)
			      ? 2
			      : 1;
		...
		for (i = dsi_num * lcd->disp, j = 0; i < CLK_DSI_NUM; ++i) {
			if (lcdp->dsi_clk[i])
				disp_sys_clk_disable((void *)lcdp->dsi_clk[i]);
			++j;
			if (j == dsi_num)
				break;
		}
	} else if (lcdp->panel_info.lcd_if == LCD_IF_EDP) {
		disp_sys_clk_disable((void *)lcdp->edp_clk);
	}

	if (hal_clock_is_enabled(lcdp->clk) == HAL_CLK_STATUS_ENABLED)
		disp_sys_clk_disable((void *)lcdp->clk);
...
static s32 disp_lcd_pin_cfg(struct disp_device *lcd, u32 bon)
{
	...
	sprintf(dev_name, "lcd%lu", lcd->disp);
...
static s32 disp_lcd_backlight_enable(struct disp_device *lcd)
{
	...
	lcd_data_lock = hal_spin_lock_irqsave();
	if (lcdp->bl_enabled) {
		hal_spin_unlock_irqrestore(lcd_data_lock);
		return -1;
	}
...
#if !defined(CONFIG_COMMON_CLK_ENABLE_SYNCBOOT)
	if (lcdp->clk &&
	   (hal_clock_is_enabled(lcdp->clk) != HAL_CLK_STATUS_ENABLED))
		ret = 0;
#endif
				...
				u32 direction)
{
	...
	sprintf(gpio_name, "lcd_gpio_%lu", io_index);
