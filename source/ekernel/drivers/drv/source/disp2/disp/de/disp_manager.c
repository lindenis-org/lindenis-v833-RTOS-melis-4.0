...
disp_lyr_set_manager(struct disp_layer *lyr, struct disp_manager *mgr)
{
	...
	mgr_data_lock = hal_spin_lock_irqsave();
	lyr->manager = mgr;
	list_add_tail(&lyr->list, &mgr->lyr_list);
	hal_spin_unlock_irqrestore(mgr_data_lock);
	...
	for (disp = 0; disp < num_screens; disp++) {
		...
		for (chn = 0; chn < num_channels; chn++) {
			...
			for (layer_id = 0; layer_id < num_layers;
				...
				sprintf(lyr->name, "mgr%lu chn%lu lyr%lu",
					disp, chn, layer_id);
				lyr->disp = disp;
				lyr->chn = chn;
				lyr->id = layer_id;
...
static s32 disp_mgr_clk_init(struct disp_manager *mgr)
{
	...
	mgrp->clk_parent = (disp_clk_t)disp_sys_clk_get_parent((void *)mgrp->clk);
	mgrp->cfg->config.de_freq = disp_sys_clk_get_rate((void *)mgrp->clk);
...
static s32 disp_mgr_clk_enable(struct disp_manager *mgr)
{
	...
	disp_sys_clk_set_parent((void *)mgrp->clk, (void *)mgrp->clk_parent);

	if (mgr->get_clk_rate && mgrp->clk) {
		DE_INF("set DE rate to %u\n", mgr->get_clk_rate(mgr));
		de_freq = mgr->get_clk_rate(mgr);
		disp_sys_clk_set_rate((void *)mgrp->clk, de_freq);
		if (de_freq != disp_sys_clk_get_rate((void *)mgrp->clk)) {
			if (mgrp->clk_parent)
				disp_sys_clk_set_rate((void *)mgrp->clk_parent, de_freq);
			disp_sys_clk_set_rate((void *)mgrp->clk, de_freq);
			if (de_freq != disp_sys_clk_get_rate((void *)mgrp->clk)) {
	...
	ret = disp_sys_clk_enable((void *)mgrp->clk);
	if (ret != 0)
		DE_WRN("fail enable mgr's clock!\n");

	if (mgrp->extra_clk) {
		ret = disp_sys_clk_enable((void *)mgrp->extra_clk);
		if (ret != 0)
			DE_WRN("fail enable mgr's extra_clk!\n");
	}

#if defined(CONFIG_ARCH_SUN50IW10)
	disp_sys_clk_set_parent((void *)mgrp->clk1, (void *)mgrp->clk_parent);

	if (mgr->get_clk_rate && mgrp->clk1) {
		DE_INF("set DE rate to %u\n", mgr->get_clk_rate(mgr));
		de1_freq = mgr->get_clk_rate(mgr);
		disp_sys_clk_set_rate((void *)mgrp->clk1, de1_freq);
		if (de1_freq != disp_sys_clk_get_rate((void *)mgrp->clk1)) {
			if (mgrp->clk_parent)
				disp_sys_clk_set_rate((void *)mgrp->clk_parent, de1_freq);
			disp_sys_clk_set_rate((void *)mgrp->clk1, de1_freq);
			if (de1_freq != disp_sys_clk_get_rate((void *)mgrp->clk1)) {
				DE_WRN("Set DE clk fail\n");
				return -1;
	...
	DE_INF("mgr %d clk enable\n", mgr->disp);
	ret = disp_sys_clk_enable((void *)mgrp->clk1);
	if (ret != 0)
		DE_WRN("fail enable mgr's clock!\n");

	if (mgrp->dpss0_clk) {
		ret = disp_sys_clk_enable((void *)mgrp->dpss0_clk);
		if (ret != 0)
			DE_WRN("fail enable mgr's dpss0_clk!\n");
	}
	if (mgrp->dpss1_clk) {
		ret = disp_sys_clk_enable((void *)mgrp->dpss1_clk);
		if (ret != 0)
...
static s32 disp_mgr_clk_disable(struct disp_manager *mgr)
{
	...
	if (mgrp->extra_clk)
		disp_sys_clk_disable((void *)mgrp->extra_clk);

	disp_sys_clk_disable((void *)mgrp->clk);

#if defined(CONFIG_ARCH_SUN50IW10)
	if (mgrp->dpss0_clk)
		disp_sys_clk_disable((void *)mgrp->dpss0_clk);
	if (mgrp->dpss1_clk)
		disp_sys_clk_disable((void *)mgrp->dpss1_clk);

	disp_sys_clk_disable((void *)mgrp->clk1);
#endif
...
static s32 disp_mgr_dump(struct disp_manager *mgr)
{
	...
	DISP_PRINT("mgr%lu: %dx%d fmt[%s] cs[0x%x] range[%s] eotf[0x%x] bits[%s] "
